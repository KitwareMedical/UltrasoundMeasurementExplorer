...
public:
  ...
  void UpdateMeasurements(IntersonArrayDeviceRF::RFImageType::Pointer input,
    ImageType::Pointer bmode);
  void DrawRectangle(VectorImageType::Pointer composite,
    itk::CurvilinearArraySpecialCoordinatesImage<double, 2>::Pointer curvedImage, int index);
  void SetRegion(int x, int y);

  void DrawMMode(IntersonArrayDeviceRF& intersonDevice, int index,
    CastDoubleFilterType::Pointer input, itk::PermuteAxesImageFilter<ImageType>::Pointer output,
    QLabel* mModeLabel);
